...
async function run () {
    ...
    let winnerMatrixes = 0;
    let winnerIndex = 0;
    let lastWinnerIndex = 0;
    while(matrixes.length > 0 && draws.length>0) {
        ...
        do {
            winnerIndex = matrixes.findIndex(m => {
                let lineWin = m.some(r => r.every(e => e.checked));
                ...
                //console.log('row win', lineWin, 'column win', columnWin);
                return lineWin || columnWin;
            });

            if (winnerIndex >= 0 && matrixes.length >= 1) {
                lastWinnerIndex = winnerIndex;
                winner = matrixes[winnerIndex];
                matrixes.splice(winnerIndex, 1);
            }
        } while (winnerIndex >= 0);

        console.log('matrix', matrixes.length, winnerIndex, matrixes.length);
    ...
    console.log('winner index', lastWinnerIndex ,'winner', winner, 'winner number', winnerNumber, 'uncheked sum', uncheckedSum, 'result', uncheckedSum * winnerNumber);
}
